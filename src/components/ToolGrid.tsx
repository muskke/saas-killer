...
export default function ToolGrid({ tools }: { tools: Tool[] }) {
  ...
  const filteredTools = useMemo(() => {
    return tools.filter((tool) => {
      ...
      // Normalized search using deferred value for performance (INP)
      ...
      <div className="mb-12 space-y-6">
        ...
        <div className="max-w-3xl mx-auto relative group mb-8">
          ...
          <div className="relative bg-white dark:bg-zinc-900 rounded-xl flex items-center p-2 shadow-xl border border-gray-100 dark:border-zinc-800 group-focus-within:border-transparent transition-colors">
            ...
            />
            {isPending && <div className="animate-spin w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full mr-2" />}
            {search && (
        ...
        <nav aria-label="Primary filter" className="flex overflow-x-auto gap-2 md:gap-3 md:justify-center -mx-4 px-4 md:mx-0 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          <button
