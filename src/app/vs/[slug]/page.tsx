...
import AdBanner from '@/components/AdBanner'; // 记得引用你的广告组件

type Props = {
  params: Promise<{ slug: string }>;
};

// 1. 生成静态路由 (Static Export 必备)
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Don't buy ${competitor} until you read this. Compare features, pricing, and privacy. See why ${tool.name} is the best open-source alternative.`,
  };
}
...
export default async function VsPage({ params }: Props) {
  ...
  const competitor = tool.rich_features?.competitor_name || 'Proprietary Software';
  // 如果数据库里没生成表格，用个兜底数据
  const comparisons = tool.rich_features?.comparison_table || [
    { feature: 'Pricing', saas_value: '$$$ Monthly Subscription', os_value: 'Free & Open Source' },
    { feature: 'Data Privacy', saas_value: 'They own your data', os_value: '100% Self-hosted' },
    { feature: 'Customization', saas_value: 'Limited', os_value: 'Full Code Access' },
  ];
  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <Link href={`/alternatives/${tool.slug}`} className="text-sm text-gray-500 hover:text-indigo-600 font-bold">
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="text-center mb-16 space-y-4">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-gray-900 leading-tight">
            <span className="text-red-500 line-through decoration-4 decoration-red-300 opacity-60">{competitor}</span>
            <span className="mx-4 text-gray-300">vs</span>
            <span className="text-indigo-600">{tool.name}</span>
          </h1>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Why pay monthly fees for {competitor} when you can self-host {tool.name} for free?
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-16 relative">
          ...
          <div className="bg-white rounded-3xl p-8 border border-gray-200 shadow-sm opacity-80 grayscale-[0.3]">
            <div className="text-center mb-8">
              ...
              <p className="text-red-500 font-medium text-sm mt-1">The Expensive Option</p>
            </div>
          ...
          <div className="bg-white rounded-3xl p-8 border-2 border-indigo-500 shadow-2xl relative overflow-hidden transform md:-translate-y-4">
            ...
            <div className="text-center mb-8">
              <h3 className="text-2xl font-bold text-indigo-700">{tool.name}</h3>
              <p className="text-green-600 font-medium text-sm mt-1">The Freedom Choice</p>
            </div>
            <ul className="space-y-6">
            ...
            <div className="mt-8 pt-8 border-t border-gray-100">
              <a href={tool.url} target="_blank" className="block w-full text-center bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg hover:shadow-indigo-200">
                Get {tool.name} Now
              </a>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden mb-16">
          ...
          <div className="divide-y divide-gray-100">
            {comparisons.map((row: any, idx: number) => (
              <div key={idx} className="grid grid-cols-3 p-6 gap-4 hover:bg-gray-50 transition-colors">
                <div className="col-span-3 md:col-span-1 font-bold text-gray-900 flex items-center gap-2">
                  {/* 根据特性名给个图标 */}
                  {idx === 0 ? <Coins size={18} className="text-amber-500"/> : 
                   idx === 1 ? <Shield size={18} className="text-indigo-500"/> : 
                ...
                <div className="col-span-3 md:col-span-2 grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="text-indigo-700 font-bold">{row.os_value}</div>
                </div>
              </div>
            ))}
          </div>
